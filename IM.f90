PROGRAM test2
    ...
    INTEGER :: dE, i, D, t, k, s, j, p, idx
    !Random number variables
    INTEGER :: a = 1664525, c = 1013904223, m = 2147483647, x
    OPEN(12, FILE = '/home/r_o_c/Modular2/IsingModel/Data/IM/IM100Data.txt')
    j = 65539
    x = 1664525
    P = 65539
    idx=0
    ...
    DO k = 1, conf
        ...
        DO t = 1, L**2
            ...
            !Energy change of flipping spin
                IF (idx == 1) THEN 
                    dE = 2*(initState(L)*s + initState(idx + 1)*s)
                ELSE IF (idx == L) THEN
                    dE = 2*(initState(idx-1)*s + initState(1)*s)
                ELSE
                    dE = 2*(initState(idx - 1)*s + initState(idx + 1)*s)
                END IF
