PROGRAM test2
    IMPLICIT NONE
    INTEGER, parameter :: L = 100, conf = 1500
    INTEGER, DIMENSION(L) :: initState, PerLat, init
    REAL, DIMENSION(L**2) :: DW, Mag, PerVec, AC
    INTEGER :: dE, i, D, t, k, s, j, p, idx,g,h
    !Random number variables
    INTEGER :: a = 1664525, c = 1013904223, m = 2147483647, x
    OPEN(12, FILE = '/home/r_o_c/Modular2/IsingModel/Data/IMRBF/IM100Data.txt')
    j = 65539
    x = 1664525
    P = 655390
    g = 655391
    ...
    DO k = 1, conf
        ...
        DO t = 1, L**2
            ...
            !Energy change of flipping spin
                IF (j == 1) THEN 
                    dE = 2*(initState(L)*s + initState(idx + 1)*s) - 2*s*h
                ELSE IF (idx == L) THEN
                    dE = 2*(initState(idx-1)*s + initState(1)*s) - 2*s*h
                ELSE
                    dE = 2*(initState(idx - 1)*s + initState(idx + 1)*s) - 2*s*h
                END IF
    ...
    END DO
    DO i = 1, L**2
        WRITE(12,*) i, DW(i) , Mag(i) , PerVec(i) , AC(i) 
    END DO
END PROGRAM test2
